"""
Bot de Telegram para Cauciones IOL - Versión V29 (Golden Master - Windows/Linux Compatible)
...
# --- FIX PARA WINDOWS EN VS CODE ---
# Fuerza la salida a UTF-8 para que no exploten los emojis
...
# Configuración de Logging visible en consola
logging.basicConfig(
...
class Config:
    ...
    # --- DONACIONES (Con .strip() para limpieza) ---
...
class TelegramLogger:
    def __init__(self, name: str = __name__): pass 
    def info(self, msj): logger.info(msj)
    def error(self, msj): logger.error(msj)
...
class Formateador:
    ...
    def reporte(analisis, objetivo, memoria_detallada: Dict[int, float], mostrar_top3: bool):
        msgs = []
        if analisis.hay_alerta_critica: msgs.append(f"🚨🚨 *¡OPORTUNIDAD EXTRAORDINARIA!* 🚨🚨\n*TASA > 100% DETECTADA: {analisis.tasa_maxima}%* \n\n")
        if mostrar_top3:
            msg = "*🏆 Top 3 Mercado:*\n\n"
...
class BotHandlers:
    ...
    async def cmd_usuarios(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if total == 0: await update.message.reply_text("📭 No hay usuarios.")
        else:
            final_msg = "\n".join(msg_lines)
    ...
    async def cmd_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        msg = (f"📊 *ESTADÍSTICAS*\n\n👥 *Total:* {total_usuarios} (VIP: {usuarios_autorizados})\n🎟️ *Tokens:* {tokens_pendientes}\n📉 *Tasas:* {top_tasas}\n🩺 *Scrape:* {ultimo_scrape}\n\n💡 Usá /usuarios para ver el detalle.")
        await update.message.reply_text(msg, parse_mode='Markdown')

    # --- DONACIONES V29 (MODULO FINAL) ---
    async def cmd_donar(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not keyboard:
            await update.message.reply_text("🚧 *En mantenimiento*.\nContactá al administrador para colaborar.", parse_mode='Markdown')
            return
    ...
    async def callback_donaciones(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            logger.info(f"🖱️ CLIC RECIBIDO: {query.data}")
            await query.answer() 
            
            if 'donar_' not in query.data: return

            if query.data == 'donar_ppay':
                msj = f"🇦🇷 *Personal Pay (Transferencia)*\n\nAlias:\n`{Config.DONAR_ALIAS_PPAY}`\n\n_Tocá el texto para copiar_"
                await query.message.reply_text(msj, parse_mode='Markdown')
                
            elif query.data == 'donar_lemon':
                msj = f"🍋 *Lemon Cash (LemonTag)*\n\nTag:\n`{Config.DONAR_LEMONTAG}`\n\n_Tocá el texto para copiar_"
                await query.message.reply_text(msj, parse_mode='Markdown')
                
            elif query.data == 'donar_usdt':
                msj = f"💎 *Donación Cripto (USDT)*\n\n⚠️ *RED: TRC20 (Tron)*\n\nAddress:\n`{Config.DONAR_USDT_TRC20}`\n\n_Tocá el texto para copiar_"
                await query.message.reply_text(msj, parse_mode='Markdown')
    ...
    # --- TAREA DE ESCANEO GLOBAL (NUCLEAR) ---
    async def recoleccion_global(self, context):
        try:
            ...
            if datos: 
                ...
                max_tasa = max(d.tasa for d in datos) if datos else 0.0
                if max_tasa >= 100:
                    logging.info(f"☢️ ALERTA NUCLEAR: {max_tasa}%")
                            ...
                            if abs(max_tasa - cfg.ultima_tasa_notificada_max) >= cfg.variacion_minima:
                                try:
                                    ...
                                    if msg.strip():
                                        ...
                                        try: await context.bot.pin_chat_message(uid, sent_msg.message_id)
                                        except: pass
                                ...
                                except Exception as e: logging.error(f"Error broadcast {uid}: {e}")
...
def main():
    ...
    # 1. BOTONES: Primero, para que nunca sean bloqueados
    ...
    # 3. CONVERSACION: Al final, porque es "glotona" (greedy) y puede comerse mensajes
    ...
    logger.info("🤖 Bot V29 Iniciado (Windows Compatible + Priority Fix)")
    app.run_polling()
